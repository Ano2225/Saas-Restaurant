...
import UserTrialStatus from './UserTrialStatut';
...
export default function Header() {
 ...
 return (
   <div className="w-full">
     {/* Wrapper pour le statut d'essai et header */}
     <div className="fixed top-0 left-0 w-full z-[999999] bg-white">
       <UserTrialStatus />
       <header className="w-full bg-white min-h-[72px] border-b">
     ...
     <div className="h-[100px]"></div>
      ...
      {/* Side Menu */}
  {isMenuOpen && (
    <div className="fixed inset-0 top-[120px] bg-black/40 backdrop-blur-[4px] z-50">
      <div className="absolute top-0 right-0 w-[300px] min-h-[calc(100vh-120px)] max-h-[calc(100vh-120px)] bg-white overflow-y-auto">
        <ul className="py-2">
          {menuItems.map((item) => (
            <li key={item.href} className="px-5">
              {item.href === '/logout' ? (
                <button
                  ...
                  className="flex items-center gap-2 py-[15px] font-semibold w-full text-[#b3b3b3] hover:text-black group"
                >
                  <span className="text-[#b3b3b3] group-hover:text-[#fbaf03] transition-colors">
                    {item.icon}
                  </span>
                  {item.label}
                </button>
